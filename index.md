...
- [[ecu-tuning-not-the-basics]] 🖼️ *84 extracted images · 87 inline placements* — Advanced hybrid-turbo follow-on: extended torque↔airflow model, Simple Wastegate, boost by gear, knock monitoring/gain, sensor scaling, upgraded LPFP/HPFP, injection window, MPI, and fuel-trim correction. Source-reference-only; project safety guardrails take precedence.
...
- [[octane-booster-and-slot-5]] — **Read before selecting map slot 5.** From R20 on, slot 5 is the octane-boosted timing map: VP Octanium **Unleaded** only (2855 is leaded and would destroy the cat and the O2 sensor), 10–11 oz per 10 US gallons, slot 4 as the everyday map and fallback. Carries the delivered-timing table and the stop signals.
...
- [[eqt-s2-timing-reverse-engineering]] — **What EQT actually did to the ignition maps**, recovered from 111k Cobb datalog samples without their cal file. `IP_IGA_BAS_IVVT_VVL_PORT_L` — Basic Ignition Angle: mid-load rows left stock to the byte, then a diagonal advance to **+6.75 °CRK at 6500 rpm / 1200 mg/stk**. Validated to 0.411 °CRK on three withheld sessions (store resolution is 0.375 °). Different box code — a reference point, not values to flash.
- [[dsg-shift-fart-calibration]] 🖼️ *2 figures · 141 measured upshifts* — **What the DSG shift "fart" is calibrated to do today**, before anything is changed. The retard is already there (to −35.6 °CRK); the fuel is what is missing — a WOT upshift cuts fuel outright, so the exhaust gets air. The lever is `ID_EFF_SCC_SP_MIN` — Minimum efficiency setpoint for single cylinder shut-off in case of gear shift, not more timing. All five tables are stock on every bin the lineage has produced.
