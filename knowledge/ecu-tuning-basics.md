...
>
> **Direction note.** These are *minimum*-lambda floors — the richest the ECU may command under cat/turbo-overheat protection. 5G0906259L_0002 ships them at **0.72–0.75**, already richer than 0.80, so writing 0.80 *raises* the floors (the lean direction). The guide's stated rationale ("reduce to fatten") therefore doesn't apply to this bin — but the guide's *value* still turned out to be right, for a different reason (below).
>
> **Decision (2026-07-20, supersedes the 2026-07-07 "left at stock" call): write 0.80.** Datalogs came back **rich** under load, and community tuners confirmed 0.80 for this bin — so leaning the floors from 0.72–0.75 up to 0.80 pulls the measured over-rich condition back toward target. This is a log-validated, per-bin call, not a blind guide fill: the mechanism above still holds (0.80 is less permissive than stock), but on *this* car that is the correction the logs ask for. Written from R03 onward via `tune.fueling.lambda_floors(0.80)`; the generic base recipe still defers (leaves them at stock) because the value is bin-specific. Earlier revisions of this note said "left at stock" — that was superseded once the rich logs came in.
