#!/usr/bin/env python3
"""Apply the ecu-tuning-basics SOP to the stock bin — TuningBasicsGuide, revision R02.

Extends R01. Same pipeline (open stock bin → stage recipe edits in memory → save
...
**R02 makes no calibration change to the bin — its saved bin is byte-identical to
R01's.** R02 is a report-honesty revision:
  ...
  * Three fuelling items are documented deliberate no-changes on this bin: the three
    lambda floors stay stock (stock 0.72-0.75 is already richer than the guide's
    0.80), and the two full-load enrichment maps are already all-1.0.

R02 keeps everything R01 did (the lambda axis re-breakpoint — see below) and the
same six limiter/fuelling writes the base recipe leaves at stock, all with targets
read directly off the guide (``knowledge/ecu-tuning-basics.md`` §280-316, §340-379)
and verified against the bin:
...
This is still **revision 2 — a starting point, not a finished calibration**: review
...
OUT_BIN_NAME = "5G0906259L_0002_BasicsGuide_R02.bin"
...
# --------------------------------------------------------------------------- #
# R02 limiter/fuelling writes — targets straight off the guide, verified in-bin.
...
# --------------------------------------------------------------------------- #
R02_SECTION_FUEL = "Fueling — Heavy-throttle pedal threshold (R02)"
R02_SECTION_LIM = "Limiters — R02 additions"
...
# Recipe SECTIONS R02 supersedes: the base recipe reports these as skip_stock /
# guard_blocked (it leaves the bytes at stock); R02 actually writes them, so its
...
R02_SUPERSEDES_SECTIONS = frozenset({
...
# Every symbol R02 snapshots for before/after PNGs (non-scalar ones render; the
...
def _snapshot_r02_targets(cal: CalFile) -> dict[str, object]:
    """Pre-edit snapshot of the R02 write targets (stock, before any edits)."""
    snaps: dict[str, object] = {}
    for sym in R02_TARGET_SYMBOLS:
        try:
...
def _apply_r02_writes(cal: CalFile) -> list[TableOutcome]:
    """Stage the six R02 limiter/fuelling writes; return their report rows.

    Runs AFTER :func:`apply_basics_sop`. None of these six collide with the base
    ...
    merged report (see :data:`R02_SUPERSEDES_SECTIONS`), so each table shows once.
    """
    outcomes: list[TableOutcome] = []
    ...
    outcomes.append(TableOutcome(
        PEDAL_THRESHOLD_SYMBOL, R02_SECTION_FUEL, OUTCOME_APPLIED,
    ...
    outcomes.append(TableOutcome(
        PRS_MAX_SYMBOL, R02_SECTION_LIM, OUTCOME_APPLIED,
    ...
    for sym in INTAKE_AIR_SYMBOLS:
        ...
        outcomes.append(TableOutcome(
            sym, R02_SECTION_LIM, OUTCOME_APPLIED,
    ...
    outcomes.append(TableOutcome(
        TQ_REF_MAX_SYMBOL, R02_SECTION_LIM, OUTCOME_APPLIED,
    ...
    outcomes.append(TableOutcome(
        AIR_CYL_SP_MAX_SYMBOL, R02_SECTION_LIM, OUTCOME_APPLIED,
...
def _merge_report(
    ...
    recipe: RecipeReport,
    r02_outcomes: list[TableOutcome],
) -> RecipeReport:
    """Merge the pre-pass, recipe, and R02 outcomes into one report.

    Recipe rows whose guide section R02 actually writes are dropped (superseded)
    ...
    """
    kept = tuple(
        o for o in recipe.outcomes if o.guide_section not in R02_SUPERSEDES_SECTIONS
    )
    return RecipeReport(tuple(lambda_outcomes) + kept + tuple(r02_outcomes))
...
def main() -> None:
    ...
    # the R02 writes run after so they supersede the recipe's skip/guard rows.
    with warnings.catch_warnings():
        ...
        r02_outcomes = _apply_r02_writes(cal)

    report = _merge_report(lambda_outcomes, recipe_report, r02_outcomes)
    ...
    if report.do_not_flash():
        print("\n  ⛔ DO NOT FLASH — see the report's coherence section. "
              "This is revision 2; review, then iterate.")
    else:
        print("\n  ✅ Coherence check passed — still review the report + PNGs and "
              "verify checksums before flashing. This is revision 2; iterate.")
