"""MainTune R21 — cut slot 5's peak-load timing, keep the top end.

R21 inherits the complete R20 calibration byte for byte and changes exactly one
table: slot 5's `Spark modifier` — map slot 5 ignition offset. Two of its eight
written rpm columns come down. Everything else in the lineage — the base
ignition maps, the knock fast loop, the wastegate feedforward, fueling,
limiters, slots 1-4, slot 5's boost curve, and the patch set — is untouched.

**What R20's logs said.** `Logs/BasicsGuide_R20/log_review.md` is the evidence.
Ten dosed-tank slot-5 pulls, scored against the eight R19 slot-4 pulls on
identical event definitions, split cleanly by rpm band:

    band        R19 ev/min   R20 ev/min   mean mg/stk   R20 modifier
...
Only 4500-5000 is a real regression: 6 events against an exposure-matched
expectation of 0.9, Poisson p ~ 0.0006, and the only band in the session that
ever went past one detected event's worth of retard (two cuts at -2.25 °CRK,
at 4615 and 4653 rpm). Every other band is a one-to-three-event difference that
does not separate from noise. The 3000-3500 rate is the highest in either
session and is **pre-existing** — R19 ran 43.2/min there with no modifier at
all — so it is not R20's to fix and R21 does not touch it.

**The counterintuitive part, and the reason the cut is where it is.** The
+3.750 °CRK apex at 5000 rpm produced *zero* knock across 9.5 s of exposure,
improving on R19's 9.7/min. The knock is at 4500-4700 rpm carrying +3.00 to
+3.30, where airmass is 1505 mg/stk against the apex band's 1423. The binding
variable is cylinder filling, not the size of the offset. So R21 cuts the
peak-load shoulder and leaves the apex alone:
...
That drops the mean offset across the 4500-4700 knock cluster from +3.15 to
+1.95 °CRK, and the two -2.25 °CRK cut points from +3.17/+3.23 to +2.02/+2.19.
The 4000 column comes down with 4500 so the interpolation into the cut does not
re-create the same offset at 4250 rpm; on its own 4000-4500 was flat between
sessions and is not itself indicted.

**Why the rpm breakpoints did not move.** A breakpoint at ~4700 would separate
the peak-load shoulder from the clean apex far better than the existing columns
can. It is not available: the grid's rpm axis `0x3CE5A` and airmass axis
`0x3CDBC` are shared by all 18 `IP_IGA_BAS_IVVT_VVL_PORT_L` — Basic ignition
angle maps *and* all five slots' `Spark modifier` grids — 37 axis references
each. Re-breakpointing would silently re-index the whole ignition calibration on
every slot, slot 1 (stock) and slot 4 (the everyday map and this revision's
control) included. The shape is expressed on the existing columns instead.

**R21 is deliberately single-variable.** Near-redline timing was considered and
declined for this revision: 6000 rpm has room inside the +5.00 °CRK delivered
ceiling (base +1.875, so the modifier could reach +3.000), but 6000-6600 is the
band that went 6.7 -> 22.1 ev/min, and raising it in the same revision as the
cut would make the next session unreadable in exactly the way R20's missing
slot-4 A/B already did. It is queued for R22, gated on R21 logging clean.

**This map is still for a dosed tank only** — pump 92 AKI with VP Octanium
Unleaded at 10-11 oz per 10 US gallons. Slot 4 remains the everyday map, the
in-drive fallback, and the A/B control, byte-identical to R19 and R20.
...
OUT_BIN_NAME = "Patched_259L_R21.bin"

#: The authoritative R20 run (Tunes/REV_LOG.md § R20) — the bin that is in the
#: car, the bin whose logs this revision is a response to, and the bin R21's
#: byte-level audit is taken against. It is also read directly, for slot 4's
#: boost curve: see `_slot4_curve_from_r20`. R20 left slot 4 byte-identical to
#: R19, so this reads the same curve R20 read, one revision closer.
R20_REFERENCE = (
...
#: the R14-R19 log record ever selected it, and R20 spent the slot on timing
#: instead. Its boost curve is slot 4's, read off the R20 reference bin (KTD4)
#: rather than retyped, so the two cannot drift apart. R21 inherits that curve
#: unchanged; only the `Spark modifier` grid moves.
BOOSTER_SLOT = 5
EVERYDAY_SLOT = 4
...
#:
#: R20 ran 1.125/1.500/2.250/3.000/3.750/2.250/1.500/1.125 and its logs indicted
#: exactly one region: 4500-5000 rpm went from 4.2 to 27.5 knock events per
#: loaded minute against the R19 slot-4 baseline (Poisson p ~ 0.0006) and was
#: the only band to exceed one detected event's retard, with -2.25 °CRK cuts at
#: 4615 and 4653 rpm. R21 brings the 4000 and 4500 columns down to +1.500.
#:
#: The 5000 rpm apex is deliberately NOT cut. It carried the session's largest
#: offset and produced zero events over 9.5 s, improving on R19's 9.7/min there.
#: Airmass is what separates the two: 1505 mg/stk mean across 4500-5000 against
#: 1423 across 5000-5500. The cut follows cylinder filling, not offset size.
#:
#: 4000 comes down with 4500 so the interpolation into the cut does not restore
#: the same offset at 4250 rpm. On its own the 4000-4500 band was flat between
#: sessions (7.9 -> 6.3 ev/min) and is not independently indicted.
#:
#: 3000/3500 are held: 3000-3500 is the highest-rate band in either session and
#: is pre-existing, running 43.2 ev/min on R19 with no modifier at all.
#:
...
#: R20's shape, kept so the build can assert R21 moves the two columns it means
#: to and no others. This is the calibration that is in the car right now.
R20_SPARK_MODIFIER_DEG = (1.125, 1.500, 2.250, 3.000, 3.750, 2.250, 1.500, 1.125)

#: The rpm columns R21 changes, by index into `SPARK_MODIFIER_RPM`.
CHANGED_COLUMNS = (2, 3)
...
#:
#: R21's map still peaks at +4.500° delivered (base +3.375 at 6500 rpm / 1400
#: mg/stk, plus the +1.125 offset) — the cut is below the peak, so the ceiling
#: is unchanged from R20. +5.0 clears it by half a degree while staying tight
#: enough to catch a transposition — the 5000 rpm offset landing in the 6500
#: column would deliver +7.125° and trip this.
...
def _slot4_curve_from_r20() -> np.ndarray:
    """Read map slot 4's boost curve off the flashed R20 bin (KTD4).
    ...
    *read*, never retyped: a transcription slip would make the A/B comparison
    ...
    two different timing maps. R20 left slot 4 byte-identical to R19, so reading
    it off the R20 bin yields the same curve R20 itself wrote to slot 5 — which
    is also why R21's audit should report this table unchanged.
    ...
    if grid.shape != (8, len(SLOT_RPM_AXIS)):
        raise RuntimeError(
            ...
            f"({S50_PUT_GRID_UIDS[EVERYDAY_SLOT]}) on the R20 bin is "
            f"{grid.shape}, not (8, {len(SLOT_RPM_AXIS)})"
        )
    _require_close(
        f"`PUT setpoint` — map slot {EVERYDAY_SLOT} boost cap: R20 rpm axis",
        axis,
        SLOT_RPM_AXIS,
        atol=1e-6,
    )
    if not np.all(grid == grid[0]):
        raise RuntimeError(
            f"`PUT setpoint` — map slot {EVERYDAY_SLOT} boost cap on the R20 "
    ...
    _require_close(
        ...
        f"off the R20 bin must be the one this script declares for slot "
        f"{EVERYDAY_SLOT}",
        grid[0],
        SLOT_AGGRESSIVE,
        atol=1.0,
...
_SLOT4_CURVE_FROM_R20 = _slot4_curve_from_r20()
...
SLOT_LABELS = {
    ...
    5: "aggressive (~26 psi), slot 4's curve read off the R20 bin — the "
       "octane-boosted timing map",
}


SUMMARY = """\
R21 inherits the complete R20 calibration — every base ignition cell, the knock
fast-loop tables, the wastegate feedforward and its re-breakpointed intake axis,
the exact Spark IAT tables, fueling, limiters, slots 1-4, slot 5's boost curve,
and the patch set — and changes **exactly one table**: slot 5's `Spark modifier`
— map slot 5 ignition offset, in two of its eight written rpm columns.

**What R20's logs said.** Ten dosed-tank slot-5 pulls, scored against R19's
eight slot-4 pulls on identical event definitions, indicted one band:
...
4500-5000 is the only band that separates from noise: 6 events against an
exposure-matched expectation of 0.9 (Poisson p ~ 0.0006), and the only band in
the session to exceed one detected event's retard, with -2.25 °CRK cuts at 4615
and 4653 rpm. The 3000-3500 rate is the highest in either session and is
pre-existing — R19 ran 43.2/min there with no modifier — so R21 does not touch
it. Every other difference is one to three events and does not separate.

**The cut, and why it is not at the peak.** The +3.750 °CRK apex at 5000 rpm
produced zero knock over 9.5 s and improved on R19's 9.7/min. The knock sits at
4500-4700 rpm carrying +3.00 to +3.30, where airmass is 1505 mg/stk against the
apex band's 1423. Cylinder filling is the binding variable, not offset size:
...
and the two -2.25 °CRK cut points from +3.17/+3.23 to +2.02/+2.19. The 4000
column comes down with 4500 so the interpolation into the cut does not restore
the same offset at 4250 rpm; 4000-4500 was flat between sessions on its own.
...
including slot 1 (stock) and slot 4 (this revision's control).

**Single-variable by design.** Near-redline timing has room inside the +5.00
°CRK delivered ceiling — 6000 rpm sits at base +1.875, so its modifier could
reach +3.000 — but 6000-6600 is the band that went 6.7 -> 22.1 ev/min, and
raising it alongside the cut would make the next session unreadable the way
R20's missing slot-4 A/B already was. Queued for R22, gated on R21 logging
clean, along with writing the 1049.97 mg/stk row so the full column value is
actually delivered at redline load (~1204 mg/stk, where the grid currently
interpolates down toward the neutral row).

Delivered timing — base plus modifier, not the offset alone — is capped at
+5.00 °CRK by `slot_spark_map`, which reads the live base map to check it. R21
still peaks at +4.500° at 6500 rpm; the cut is below the peak, so the ceiling is
unchanged. The 1200 mg/stk row is written identically to the 1400 row because
WOT reaches ~1600 mg/stk, above the grid's top breakpoint, and a flat top row
makes the clamp-versus-extrapolate question above 1400 moot.
...
Slot 4 remains the everyday map, the in-drive fallback, and the A/B control,
byte-identical to R19 and R20. Selecting slot 5 on plain 92 will knock; that is
accepted and controlled by discipline, not by calibration.

R20's validation gate asked for an interleaved within-session slot-4/slot-5 A/B
and did not get one — all ten logged pulls were slot 5, so R20 had to be scored
cross-session against R19 with day and fuel as confounds. R21 asks for that A/B
again, and it is what makes the cut readable. See `Tunes/REV_LOG.md` § R21.

Every other declaration remains identical to R20. This is a starting point for
...
def _apply_r21_slot5_timing(tune: Tune) -> np.ndarray:
    ...
    is, that it differs from R20's shape in exactly the two columns R21 means to
    move, that slot 5 starts neutral, and that no *other* slot and none of the
    ...
    R21 is not an A/B against anything.
    """
    ...
    if not np.all(offsets > 0.0):
        raise RuntimeError(
            ...
            "advance; R21 does not retard slot 5 anywhere"
        )
    # R21 is a cut, not a reshape: prove it differs from the calibration that
    # is in the car in exactly the declared columns, and only downward. A
    # transposed or mis-indexed edit that happened to stay storable would
    # otherwise reach the ECU looking like a deliberate shape.
    previous = np.asarray(R20_SPARK_MODIFIER_DEG, dtype=np.float64)
    if previous.shape != offsets.shape:
        raise RuntimeError(
            "`Spark modifier` — map slot 5 ignition offset: R20 shape has "
            f"{previous.size} offsets against R21's {offsets.size}"
        )
    moved = tuple(int(i) for i in np.flatnonzero(~np.isclose(offsets, previous)))
    if moved != tuple(CHANGED_COLUMNS):
        raise RuntimeError(
            "`Spark modifier` — map slot 5 ignition offset: R21 changes rpm "
            f"columns {[SPARK_MODIFIER_RPM[i] for i in moved]}, but declares "
            f"{[SPARK_MODIFIER_RPM[i] for i in CHANGED_COLUMNS]}"
        )
    if not np.all(offsets[list(CHANGED_COLUMNS)] < previous[list(CHANGED_COLUMNS)]):
        raise RuntimeError(
            "`Spark modifier` — map slot 5 ignition offset: R21 is a cut; every "
            "changed column must come down from R20, not go up"
        )
    ...
    tune.switchpatch.slot_spark_map(
        ...
        intent="cut slot 5's peak-load ignition advance from the R20 log "
               "evidence: the 4000 and 4500 rpm columns come down to +1.500 "
               "°CRK because 4500-5000 rpm ran 27.5 knock events per loaded "
               "minute against R19 slot 4's 4.2 (p ~ 0.0006) with both -2.25 "
               "°CRK cuts, at 1505 mg/stk; the +3.750 apex at 5000 rpm is held "
               "because it logged zero events, and 5500-6500 is held so R21 "
               "tests the cut alone",
    )
    ...
    for name, before in base_before.items():
        if not np.array_equal(before, tune.values(name)):
            raise RuntimeError(
                ...
                "ignition maps must be byte-identical to R20 or slot 4 moved too"
            )
...
def declare(tune: Tune) -> tuple[float, np.ndarray]:
    ...
    for slot, curve in SLOT_CURVES.items():
        tune.switchpatch.slot_curve(
            ...
            intent=(
                ...
                f"R20 bin's slot {EVERYDAY_SLOT} ({SLOT_LABELS[slot]}), "
    ...
    delivered = _apply_r21_slot5_timing(tune)
...
def main() -> None:
    ...
    result = build(
        ...
        title="TUNE_MainTune_R21 — cut slot 5's peak-load timing, keep the top end",
        summary=SUMMARY,
    )
    ...
    print(f"R21 saved  : {result.bin_path}")
    print(f"R21 report : {result.report_path}")
    print(f"R21 journal: {len(result.journal)} entries; "
          f"{len(result.journal.tables_touched())} tables touched")
    print(f"R21 audit  : {result.diff.summary()}")
    print(f"R21 Reference IGA migration max deviation: "
          f"{reference_iat_deviation:.6f}°CRK")
    print("R21 slot 5 delivered timing at 1400 mg/stk (°CRK): "
          + ", ".join(f"{rpm:g}:{deg:+.3f}"
                      for rpm, deg in zip(SPARK_MODIFIER_RPM, delivered)))
    print("R21 cut vs R20 (°CRK offset): "
          + ", ".join(
              f"{SPARK_MODIFIER_RPM[i]:g}:{SPARK_MODIFIER_DEG[i] - R20_SPARK_MODIFIER_DEG[i]:+.3f}"
              for i in CHANGED_COLUMNS))
    print(f"R21 slot 5 boost curve copied from slot {EVERYDAY_SLOT} of "
          f"{R20_REFERENCE.name}: peak "
          f"{_SLOT4_CURVE_FROM_R20.max():.0f} hPa absolute")
    print(
        "\nReview the R20-to-R21 comparison plot for `Spark modifier` — map "
        "slot 5 ignition offset before any human-performed CAL flash. It is "
        "the ONLY table that may differ from R20: slot 5's `PUT setpoint` — "
        "map slot 5 boost cap, the nine `IP_IGA_BAS_IVVT_VVL_PORT_L[STND][i][e]"
        "` — Basic ignition angle maps, and every slot 4 table must show no "
        "change at all, or slot 4 is no longer the control this revision is "
        "measured against.\n"
        ...
        "fallback. See Tunes/REV_LOG.md § R21 for the A/B logging gate — R20 "
        "was logged slot-5-only and had to be scored cross-session; R21 needs "
        "the interleaved slot-4/slot-5 pulls to be readable.\n"
