"""MainTune R24 — a low-torque map slot 2 with a linear powerband peaking near 6000 rpm.

R24 inherits the complete R23 calibration and rebuilds **map slot 2**, which
through R23 was a "conservative ~24.5 psi" curve: a scaled-down copy of the
everyday map with no character of its own. R24 gives it one. It becomes the
low-torque map — a deliberately different engine below 5000 rpm, with a power
curve that rises in a straight line and peaks at ~6250 rpm instead of 5500.

**The shape falls out of the physics, not out of a fit.** Torque on this engine
tracks airmass per stroke, so power is proportional to ``airmass x rpm``. A
powerband that is linear in rpm is therefore, exactly, a **constant airmass**
across the range, and the power peak lands at the highest rpm the turbo can
still hold that airmass at. So the calibration problem reduces to one number —
the airmass to hold — and one measurement: how much boost this engine needs to
make it at each rpm.

That measurement comes from the same 55-log aggressive-curve population R23 was
sized against (`Logs/aggressive_slot_lineage/`), using only samples where the
boost cap is binding so spool transients do not contaminate it.
`size_r24_linear.py` inverts it. Holding **1200 mg/stk**:

    peak power      5500 rpm  ->  6250 rpm
    peak power                    -2.5 %
    torque at 4100 rpm            -22.0 %
    linearity of power vs rpm     R^2 0.9993  (the aggressive curve: 0.8147)

Peak power barely moves because the whole cut is below 5900 rpm, which is not
where peak power is.

**The aggressive curve is the ceiling, and that is what puts the peak at 6000.**
Above ~6150 rpm, holding 1200 mg/stk would need more boost than this car has
ever been logged running, because volumetric efficiency falls away faster than
the target needs. Asking for that is asking the turbo for an unlogged operating
point, so the new cap is clamped, cell by cell, at map slot 3's curve read off
the R23 bin. Airmass therefore holds flat to ~6100 rpm and then falls with the
taper — which is precisely what makes the power peak land where it was asked
to.

**Slot 2 comes out of the lambda hold.** Because of that clamp, slot 2 runs
slot 3's boost above ~6100 rpm. R23 held slots 1 and 2 at their prior, leaner
0.800 lambda there — exactly the top-end condition R23 enriched slot 3 to fix.
Leaving slot 2 held would give the new map slot 3's boost on the old fuelling,
so its `Lambda modifier` — map slot lambda offset grid is declared back to the
patch's neutral 0.00 and slot 2 takes the enrichment with slots 3, 4 and 5.
Slot 1 stays held, so R23's unresolved sign question still has its reference.

**The ladder stops being monotonic in boost, and that is the reviewable point.**
Through R23 slot 1 sat at or below every other slot at every breakpoint. The new
slot 2 is well below slot 1 through the mid-range and above it from ~6000 rpm,
so that invariant no longer holds and is replaced by three that do: slot 1 stays
at or below slots 3, 4 and 5 everywhere; slot 2 stays at or below slot 3
everywhere; and slot 2's highest cap anywhere stays below slot 1's, so the
low-torque map is still the lowest-demand map in the ladder by peak. All three
are asserted before anything is written.

**And it finishes a job R23 started.** R23 enriched 5504-7008 rpm to lambda
0.780 on the 1200.01 and 1389.00 mg/stk rows, on the reasoning that loaded WOT
runs 1200-1600 mg/stk. That holds to about 6100 rpm and fails above it: the
engine is out of breath at the top and WOT filling falls to 1176 mg/stk at 6250
rpm and 1089 at 6500, *below* the 1200.01 breakpoint, so the lookup interpolates
down toward the 1100.01 row that R22 left at 0.810/0.800. The command came out
0.786 at 6250 rpm and 0.801 at 6500 rather than 0.780 — R23's top-end enrichment
faded out over the last 400 rpm of the rev range, on every slot, in the band
carrying three of the lineage's -3.00 CRK events. R24 writes the same 0.780 onto
the 1100.01 row in the same four columns and closes it. The pump has 25-31
points of headroom there, so this is the one enrichment in the lineage that is
not pump-limited. Slot 1 is not held on that row: the offsets it would need
deliver 0.7305 and 0.7510 lambda under the wrong sign convention, past the 0.755
rich bound, and `_check_row5_hold_refused` proves the guard refuses them rather
than leaving that as a claim in a comment.

Nothing else moves. Slots 1, 3, 4 and 5 keep their boost caps, read off the R23
bin and written back; the base ignition maps, the knock fast-loop tables, the
wastegate feedforward, the Spark IAT family, the limiters and every `Spark
modifier` grid are R23's exactly, and in the lambda grids only row 5 moves.
Five tables differ from R23.
...
Revision history (full rationale in Tunes/REV_LOG.md):
    ...
    R23 — Optimise the pump-92 aggressive slot: enrich 3000-3500 rpm and the
         ...
         modifier` grids. Boost unchanged. Built and verified.
    R24 — Rebuild map slot 2 as the low-torque map: a flat 1200 mg/stk airmass
         target, which is a linear powerband, peaking at ~6250 rpm instead of
         5500, clamped at slot 3's curve above ~6150 rpm. Slot 2 leaves the
         lambda hold and takes R23's enrichment. Also extends R23's 5504-7008
         rpm enrichment onto the 1100.01 mg/stk row, which WOT filling falls
         onto above ~6100 rpm, so it stops fading out at the top. Five tables.
"""
...
OUT_BIN_NAME = "Patched_259L_R24.bin"

#: The authoritative R23 run (Tunes/REV_LOG.md § R23) — the lineage tip, the
#: parent of this revision, and the bin R24's byte-level audit is taken against.
#:
#: R23 was built and verified but not flashed, so the car still holds R22. That
#: is deliberate and it costs nothing: R24 changes only map slot 2, and R23
#: changed only the shared lambda grids and slot 3, so a slot 3 pull on an R24
#: bin is still exactly a test of R23. See Tunes/REV_LOG.md § R24.
#:
#: It is also read directly, for four of the five boost curves: see
#: `_curve_from_r23`. R24 changes exactly one boost curve, and the cheapest way
#: to *prove* the other four are untouched is to read each cap off the parent
#: bin and write it back, rather than re-derive them from constants and hope the
#: arithmetic lands on the same bytes. Slot 3's curve read this way is also what
#: bounds the new slot 2 from above.
R23_REFERENCE = (
    REPO_ROOT / "Tunes" / "MainTune" / "MainTune_out"
    / "R23_20260903-111406"
    / "Patched_259L_R23.bin"
)
R23_REFERENCE_SHA256 = (
    "c43f67d5c9a646ac31adf12311fd5913e0457abf4e0de08e61bc08624a7c783a"
)
...
#: R24's correction to R23's top-end enrichment: the same 0.780, extended down
#: to the **1100.01 mg/stk row** in the same top-end columns.
#:
#: R23 wrote 0.780 at 5504-7008 rpm on the 1200.01 and 1389.00 rows only, on the
#: reasoning that loaded WOT runs 1200-1600 mg/stk. That is true up to about
#: 6100 rpm and false above it: the engine is out of breath at the top of the
#: rev range, and WOT filling on the aggressive curve falls to **1176 mg/stk at
#: 6250 rpm and 1089 at 6500** as logged. Both are *below* the 1200.01
#: breakpoint, so the lookup interpolates down toward the 1100.01 row — which
#: R22 left at 0.810/0.800 — and the commanded value comes out **0.786 at 6250
#: rpm and 0.801 at 6500** rather than the 0.780 R23 intended.
#:
#: So R23's top-end enrichment fades out over the last 400 rpm of the rev range,
#: on every slot, in the band carrying three of the lineage's -3.00 CRK events.
#: Writing the row below closes it. The pump has 25-31 points of headroom there
#: (`HPFP Eff Vol` median 75.6 % at 5500-6000 and 69.3 % at 6000-6600), so this
#: is the one enrichment in the lineage that is not pump-limited.
#:
#: **3008 rpm is deliberately not in this set.** At that rpm WOT runs 1382
#: mg/stk on the aggressive curve and 1200 on the low-torque map, both at or
#: above the 1200.01 row, so the 1100.01 row is never reached under load there.
#: It also holds 1.000, so enriching it to 0.930 would be a large enrichment of
#: a genuinely part-load cell that nothing has asked for.
LAMBDA_R24_TOP_END_ROW = 5
LAMBDA_R24_TOP_END_RPM = (5504.0, 5984.0, 6496.0, 7008.0)
...
def _lambda_cells_r24() -> tuple[tuple[float, ...], ...]:
    """R22's grid, R23's enrichment, and R24's extension of it one row down."""
    rows = [list(row) for row in LAMBDA_CELLS_R22]

    def enrich(rpm: float, value: float, row: int, why: str) -> None:
        ...
        if value > rows[row][column] + 1e-12:
            raise RuntimeError(
                ...
                f"{rows[row][column]:.4f} to {value:.4f} — leaner, not richer. "
                f"{why} declares an enrichment; refusing."
            )
        rows[row][column] = value

    for rpm, value in LAMBDA_R23_ENRICHED.items():
        for row in LAMBDA_LOADED_ROWS:
            enrich(rpm, value, row, "R23")

    # R24: the same top-end value, one row further down, so the enrichment
    # survives the filling falling off the 1200.01 breakpoint above ~6100 rpm.
    # The value is taken from R23's own declaration rather than retyped, so the
    # two rows cannot drift apart.
    for rpm in LAMBDA_R24_TOP_END_RPM:
        enrich(rpm, LAMBDA_R23_ENRICHED[rpm], LAMBDA_R24_TOP_END_ROW, "R24")

    # The rows the top-end write spans must now agree, or the enrichment still
    # has a step in it for the lookup to interpolate across.
    for rpm in LAMBDA_R24_TOP_END_RPM:
        column = LAMBDA_RPM.index(int(rpm))
        span = {rows[r][column]
                for r in (LAMBDA_R24_TOP_END_ROW, *LAMBDA_LOADED_ROWS)}
        if len(span) != 1:
            raise RuntimeError(
                f"`IP_LAMB_BAS_HPDI[1]` at {rpm:g} rpm: rows "
                f"{(LAMBDA_R24_TOP_END_ROW, *LAMBDA_LOADED_ROWS)} hold {span}, "
                "not one value. The top-end enrichment would still fade as "
                "filling crosses a breakpoint."
            )
    return tuple(tuple(row) for row in rows)


LAMBDA_CELLS = _lambda_cells_r24()

#: The slots held back at their prior lambda through the per-slot `Lambda
#: modifier` grids. R23 held slots 1 and 2; **R24 holds slot 1 only.**
#:
#: Slot 2 comes out because of the clamp on its new boost curve: above ~6150 rpm
#: the low-torque map runs slot 3's boost, and R23's hold would leave it there
#: on the old, leaner 0.800 lambda — exactly the top-end condition R23 enriched
#: slot 3 to fix. A map that runs the aggressive curve's boost gets the
#: aggressive curve's fuelling. Slot 2's grid is therefore declared back to the
#: patch's neutral 0.00 (see `RELEASE_LAMBDA_SLOTS`), and it takes the
#: enrichment with slots 3, 4 and 5.
#:
#: Slot 1 stays held, so R23's unresolved question — which way a positive offset
#: moves the mixture, never yet observed on this car — keeps its reference, and
#: keeps it on the lowest-boost map in the ladder, where a wrong sign is a rich
#: failure with the most fuel-pump headroom behind it.
#:
#: **The hold stays on rows 6 and 7 and does not follow R24's top-end write down
#: to row 5.** Holding row 5 at what R22 had there would need offsets of +0.050
#: and +0.030 at 5504 and 5984 rpm, and under the *wrong* sign convention those
#: deliver 0.730 and 0.750 lambda — past the 0.755 rich bound, beyond which the
#: pump cannot hold rail pressure and extra enrichment stops arriving. The bound
#: refuses them, which is the guard working rather than an oversight, so slot 1
#: takes the top-end enrichment at 1100 mg/stk along with every other slot. That
#: is the safe direction on the bad-tank map, and `_check_row5_hold_refused`
#: below proves the refusal rather than leaving it as a claim in a comment.
HOLD_LAMBDA_SLOTS = (1,)

#: Slots whose `Lambda modifier` grid R24 declares back to the patch's neutral
#: 0.00. Declaring the write matters: the grid is neutral on a freshly patched
#: bin, so writing 0.00 stages no bytes — but it *declares* the table's extent,
#: which is what lets the raw-diff audit attribute the difference against the
#: R23 bin, where these cells are non-zero, instead of calling it unexplained.
#: An omitted write would fail the build, and rightly.
RELEASE_LAMBDA_SLOTS = (2,)
...
def _check_row5_hold_refused() -> tuple[float, ...]:
    """Prove the row-5 hold really is refused by the both-signs rich bound.

    The comment on `HOLD_LAMBDA_SLOTS` says slot 1 cannot be held at R22's
    lambda on the 1100.01 mg/stk row because the offsets it would need are
    unsafe under the wrong sign convention. A comment cannot rot quietly if the
    build checks it, so this computes those offsets and asserts that at least
    one of them does breach the bound. If a future revision changes the bound or
    the grid such that the hold becomes safe, this fails and forces the comment
    to be revisited rather than silently becoming false.

    Returns the delivered lambda each row-5 hold would produce under the wrong
    sign, for the report.
    """
    rich, _lean = DELIVERED_LAMBDA_RANGE
    wrong_sign = []
    for rpm in LAMBDA_R24_TOP_END_RPM:
        column = LAMBDA_RPM.index(int(rpm))
        prior = _quantise_lambda(LAMBDA_CELLS_R22[LAMBDA_R24_TOP_END_ROW][column])
        enriched = _quantise_lambda(LAMBDA_CELLS[LAMBDA_R24_TOP_END_ROW][column])
        wrong_sign.append(enriched - (prior - enriched))
    if not any(value < rich for value in wrong_sign):
        raise RuntimeError(
            "`Lambda modifier` — map slot 1 lambda offset: holding the "
            f"{LAMBDA_LOAD[LAMBDA_R24_TOP_END_ROW]:.2f} mg/stk row at R22's "
            "lambda would now be safe under both sign conventions "
            f"(wrong-sign delivered {['%.4f' % v for v in wrong_sign]}, rich "
            f"bound {rich}). The reason given for leaving slot 1 unheld there "
            "no longer holds; revisit HOLD_LAMBDA_SLOTS."
        )
    return tuple(wrong_sign)
...
#: the R23 bin into slot N. There is no permutation this time and no boost change
...
def _curve_from_r23(slot: int, expected: np.ndarray, label: str) -> np.ndarray:
    ...
    the read is checked rather than trusted; a mismatch means the R23 bin is not
    ...
    """
    cal = CalFile.open(
        str(SWITCH_XDF), str(R23_REFERENCE), structure=structure_of(R23_REFERENCE)
    )
    ...
    if grid.shape != (8, len(SLOT_RPM_AXIS)):
        raise RuntimeError(
            ...
            f"({S50_PUT_GRID_UIDS[slot]}) on the R23 bin is "
            f"{grid.shape}, not (8, {len(SLOT_RPM_AXIS)})"
        )
    _require_close(
        f"`PUT setpoint` — map slot {slot} boost cap: R23 rpm axis",
        axis,
        SLOT_RPM_AXIS,
        atol=1e-6,
    )
    if not np.all(grid == grid[0]):
        raise RuntimeError(
            f"`PUT setpoint` — map slot {slot} boost cap on the R23 bin is not "
    ...
    _require_close(
        ...
        f"off the R23 bin must be the one this script declares",
        grid[0],
        expected,
        atol=1.0,
    )
...
#: Every slot's curve, read off the R23 bin rather than retyped. R23's whole
#: boost claim is "unchanged", and this is what makes that provable.
#: The airmass the low-torque map holds flat, in mg/stk. This single number is
#: the whole calibration: power goes as airmass x rpm, so a constant airmass is
#: a power curve linear in rpm. Chosen with Sam on 2026-09-03 from a sweep in
#: `Logs/aggressive_slot_lineage/size_r24_linear.py` — 1200 gives the
#: straightest line of the candidates (R^2 0.9993), drops mid-range torque ~22 %
#: and costs ~2.5 % of peak power, because the cut is entirely below 5900 rpm.
LINEAR_TARGET_MG = 1200.0

#: The boost the engine needs to hold `LINEAR_TARGET_MG` at each breakpoint of
#: the shared 12-point slot rpm axis, in absolute hPa, **before** the clamp
#: below. Derived by `size_r24_linear.py` from measured volumetric efficiency —
#: mg/stk per kPa of manifold pressure, taken over the 55-log aggressive-curve
#: population using only samples where the cap was binding — and reproduced by
#: running that script. It is not a curve shape anyone drew: it is
#: `LINEAR_TARGET_MG / VE(rpm)`, which is why it dips in the middle (VE peaks at
#: ~5000 rpm, so the least boost is needed there) and rises at both ends.
LINEAR_REQUIRED_HPA = np.asarray(
    (2327.0, 2318.0, 2294.0, 2219.0, 2197.0, 2197.0,
     2168.0, 2191.0, 2290.0, 2345.0, 2384.0, 2472.0),
    dtype=np.float64,
)

SLOT_CURVES = {
    1: _curve_from_r23(1, _slot1_curve(SLOT_RPM_AXIS), "slot 1 composed"),
    CONTROL_SLOT: _curve_from_r23(
        CONTROL_SLOT, SLOT_AGGRESSIVE, "aggressive ~26 psi"),
    MID_BOOSTER_SLOT: _curve_from_r23(
        MID_BOOSTER_SLOT, SLOT_INTERMEDIATE, "intermediate ~24.4 psi"),
    BOOSTER_SLOT: _curve_from_r23(
        BOOSTER_SLOT, SLOT_AGGRESSIVE, "aggressive ~26 psi"),
}

#: What slot 2 held under R23, read off the parent bin. R24 does not write this
#: curve anywhere — it is read so the revision can *prove* what it is replacing,
#: and so a parent bin whose slot 2 was not the conservative curve fails the
#: build instead of being silently overwritten.
_R23_SLOT2_CONSERVATIVE = _curve_from_r23(
    2, SLOT_CONSERVATIVE, "conservative ~24.5 psi (the curve R24 replaces)")

#: The shipped low-torque curve: the required curve, clamped cell by cell at the
#: aggressive curve **as read off the parent bin**.
#:
#: Above ~6150 rpm, holding 1200 mg/stk would need more boost than this car has
#: ever been logged running — VE falls away faster than the target needs — and
#: asking for it would be asking the turbo for an unlogged operating point. The
#: clamp is taken against the bin-read curve rather than against the declared
#: constant so that "slot 2 never exceeds slot 3" is true of the bytes, not of
#: the arithmetic; quantisation in the stored aggressive curve cannot open a gap
#: this way. The clamp is also what puts the power peak at ~6250 rpm: airmass
#: holds flat to ~6100 and then falls with slot 3's taper.
SLOT_LINEAR_LOW_TORQUE = np.minimum(
    LINEAR_REQUIRED_HPA, SLOT_CURVES[CONTROL_SLOT])
SLOT_CURVES[2] = SLOT_LINEAR_LOW_TORQUE

_R23_AGGRESSIVE = SLOT_CURVES[CONTROL_SLOT]
_R23_INTERMEDIATE = SLOT_CURVES[MID_BOOSTER_SLOT]
SLOT_LABELS = {
    ...
    2: "LOW TORQUE, linear powerband — a flat 1200 mg/stk airmass target "
       "(~16.8-19.3 psi, a shallow bowl rather than a shelf), peaking at "
       "~6250 rpm, clamped at slot 3's curve above ~6150 rpm, pump 92, ENRICHED",
    CONTROL_SLOT: "aggressive (~26 psi), pump 92, ENRICHED, +0.750 °CRK at 3500 "
                  "and 4000 rpm — the everyday map and the in-drive fallback, "
                  "exactly as R23 built it",
...
SLOT_CURVE_INTENTS = {
    slot: (f"retain map slot {slot}'s boost cap exactly as R23 built it, read "
           f"off the R23 bin rather than retyped ({SLOT_LABELS[slot]})")
    for slot in (1, CONTROL_SLOT, MID_BOOSTER_SLOT, BOOSTER_SLOT)
}
SLOT_CURVE_INTENTS[2] = (
    "rebuild map slot 2 as the low-torque map: the boost that holds a flat "
    f"{LINEAR_TARGET_MG:.0f} mg/stk airmass at every breakpoint, which is a "
    "power curve linear in rpm, clamped at slot 3's curve above ~6150 rpm so "
    "the cap never asks for an operating point this car has not been logged "
    f"running ({SLOT_LABELS[2]})"
)
...
def _check_slot_ladder() -> tuple[float, float]:
    """Prove R24 moves map slot 2 and nothing else in the ladder, before writing.

    Slots 1, 3, 4 and 5 were read off the R23 bin, so for those the interesting
    failures are a mis-declared *expectation* (caught in `_curve_from_r23`) and
    a slot that stopped standing in the relationship the labels claim.

    Slot 2 is the one curve this revision authors, and it **breaks the ladder
    invariant every revision since R14 has carried**: that slot 1 sits at or
    below every other slot at every breakpoint. The low-torque curve is far
    below slot 1 through the mid-range and above it from ~6000 rpm, because it
    is not a scaled-down copy of anything — it is a different shape. So that
    invariant is retired here, deliberately and in the open, and replaced by
    three that carry the safety it was standing in for:

    * slot 1 stays at or below slots 3, 4 and 5 at every breakpoint, so the
      bad-tank map is still never above a performance map;
    * slot 2 stays at or below slot 3 at every breakpoint, so the low-torque map
      is bounded everywhere by the curve this car has 55 logged pulls on;
    * slot 2's highest cap anywhere stays below slot 1's, so the low-torque map
      is still the lowest-demand map in the ladder measured by peak, which is
      the number that sets peak cylinder pressure.

    Returns the peak and maximum boost separation in hPa, for the report.
    """
    if not np.array_equal(SLOT_CURVES[2], SLOT_LINEAR_LOW_TORQUE):
        raise RuntimeError(
            "slot 2 must carry the low-torque curve this revision authors"
        )
    if np.array_equal(SLOT_CURVES[2], _R23_SLOT2_CONSERVATIVE):
        raise RuntimeError(
            "slot 2's new curve is identical to the conservative curve it "
            "replaces; R24 would change nothing"
        )
    if not np.array_equal(SLOT_CURVES[CONTROL_SLOT], _R23_AGGRESSIVE):
        raise RuntimeError(
            f"slot {CONTROL_SLOT} must carry the aggressive curve read off the "
            f"R23 bin's slot {CONTROL_SLOT}"
        )
    if not np.array_equal(SLOT_CURVES[BOOSTER_SLOT], _R23_AGGRESSIVE):
        raise RuntimeError(
    ...
    if not np.array_equal(SLOT_CURVES[MID_BOOSTER_SLOT], _R23_INTERMEDIATE):
        raise RuntimeError(
            f"slot {MID_BOOSTER_SLOT} must carry the intermediate curve read "
            f"off the R23 bin's slot {MID_BOOSTER_SLOT}"
        )

    # Slot 1 is the map you reach for when something is wrong, so it has to be
    # at or below every *performance* slot at every breakpoint. Composing it
    # from two curves rather than reading one makes that worth checking. Slot 2
    # is deliberately not in this set any more — see the docstring.
    for other in (CONTROL_SLOT, MID_BOOSTER_SLOT, BOOSTER_SLOT):
    ...
    # The low-torque map is bounded everywhere by the curve with 55 logged
    # pulls behind it. This is the check the clamp in `SLOT_LINEAR_LOW_TORQUE`
    # exists to satisfy, restated against the values actually about to be
    # written rather than against the arithmetic that produced them.
    over = [
        f"{rpm:g} rpm: {two:.0f} > {three:.0f} hPa"
        for rpm, two, three in zip(SLOT_RPM_AXIS, SLOT_CURVES[2],
                                   SLOT_CURVES[CONTROL_SLOT])
        if two > three + 1e-9
    ]
    if over:
        raise RuntimeError(
            "map slot 2 is the low-torque map and must stay at or below slot "
            f"{CONTROL_SLOT}'s aggressive curve, but exceeds it at "
            f"{', '.join(over)}"
        )

    # And it must still be the lowest-demand map in the ladder by peak, which is
    # what sets peak cylinder pressure. Slot 2 rises above slot 1 at the top of
    # the range by design; it must not rise above slot 1's *highest* cap.
    if float(SLOT_CURVES[2].max()) >= float(SLOT_CURVES[1].max()):
        raise RuntimeError(
            f"map slot 2 peaks at {SLOT_CURVES[2].max():.0f} hPa, at or above "
            f"slot 1's {SLOT_CURVES[1].max():.0f} hPa. The low-torque map must "
            "stay the lowest-demand map in the ladder by peak boost"
        )
...
SUMMARY = """\
R24 inherits the complete R23 calibration — the enriched base lambda grids, the
base ignition cells, the knock fast-loop tables, the wastegate feedforward and
its re-breakpointed intake axis, the exact Spark IAT tables, the limiters, the
patch set, and every `Spark modifier` — map slot ignition offset grid — and
rebuilds **map slot 2**.

**What slot 2 becomes.** Through R23 it was a "conservative ~24.5 psi" curve: a
scaled-down copy of the everyday map with no character of its own. R24 makes it
the low-torque map — a deliberately different engine below 5000 rpm, with a
power curve that rises in a straight line and peaks at ~6250 rpm instead of
5500.

**The shape falls out of the physics, not out of a fit.** Torque tracks airmass
per stroke, so power goes as airmass x rpm. A powerband linear in rpm is
therefore, exactly, a constant airmass — and the peak lands at the highest rpm
the turbo can still hold it. The whole calibration is one number, 1200 mg/stk,
plus the measurement of how much boost this engine needs to make it:

    rpm   VE mg/kPa   cap hPa   psi g   was    airmass
    3000      5.157      2327   19.05  20.38      1200
    3400      5.230      2294   18.59  24.95      1200
    3800      5.408      2219   17.49  26.01      1200
    4400      5.462      2197   17.17  25.51      1200
    5000      5.535      2168   16.75  23.74      1200
    5400      5.476      2191   17.09  22.39      1200
    5750      5.241      2290   18.52  21.27      1200
    6000      5.117      2345   19.32  20.44      1200
    6250      5.033      2335   19.17  19.58      1175   <- clamped at slot 3
    6500      4.855      2243   17.84  19.16      1089   <- clamped at slot 3

Volumetric efficiency is measured over the same 55-log aggressive-curve
population R23 was sized against (`Logs/aggressive_slot_lineage/`), using only
samples where the boost cap was binding so spool transients do not contaminate
it. The cap is then `1200 / VE(rpm)`. Nobody drew this curve: it dips in the
middle because VE peaks near 5000 rpm, so that is where the least boost is
needed. `size_r24_linear.py` reproduces every number.
...
Peak power barely moves because the entire cut is below 5900 rpm, which is not
where peak power is. Below ~3400 rpm the cap was not binding before and the
turbo is still spooling, so the linear region starts there.

**The aggressive curve is the ceiling, and that is what puts the peak at 6000.**
Above ~6150 rpm, holding 1200 mg/stk would need more boost than this car has
ever been logged running — VE falls away faster than the target needs — and
asking for it would be asking the turbo for an unlogged operating point. The cap
is clamped cell by cell at slot 3's curve **as read off the R23 bin**, so the
bound is true of the bytes rather than of the arithmetic. Airmass holds flat to
~6100 rpm and then falls with slot 3's taper, which is what lands the peak where
it was asked for.

**Slot 2 leaves the lambda hold.** Because of that clamp it runs slot 3's boost
above ~6150 rpm, and R23 held slots 1 and 2 at their prior, leaner 0.800 lambda
there — exactly the top-end condition R23 enriched slot 3 to fix. So slot 2's
`Lambda modifier` — map slot lambda offset grid is declared back to the patch's
neutral 0.00 and it takes the enrichment with slots 3, 4 and 5. A map running
the aggressive curve's boost gets the aggressive curve's fuelling. Slot 1 stays
held, so R23's unresolved sign question keeps its reference on the lowest-boost
map in the ladder.

**The ladder stops being monotonic in boost. This is the reviewable point.**
Every revision since R14 carried the invariant that slot 1 sits at or below
every other slot at every breakpoint. The new slot 2 is well below slot 1
through the mid-range and above it from ~6000 rpm, because it is a different
shape rather than a scaled copy. That invariant is retired here, in the open,
and replaced by three that carry the safety it stood for — all asserted before
anything is written:

* slot 1 stays at or below slots 3, 4 and 5 everywhere, so the bad-tank map is
  never above a performance map;
* slot 2 stays at or below slot 3 everywhere, so the low-torque map is bounded
  by the curve with 55 logged pulls behind it;
* slot 2's highest cap anywhere stays below slot 1's, so it is still the
  lowest-demand map in the ladder measured by peak — the number that sets peak
  cylinder pressure.

**R24 also finishes R23's top-end enrichment.** R23 wrote lambda 0.780 at
5504-7008 rpm on the 1200.01 and 1389.00 rows, because loaded WOT runs 1200-1600
mg/stk. That is true to ~6100 rpm and false above it: WOT filling falls to 1176
mg/stk at 6250 rpm and 1089 at 6500, *below* the 1200.01 breakpoint, so the
lookup interpolates down toward the 1100.01 row R22 left at 0.810/0.800 and the
command came out **0.786 at 6250 rpm and 0.801 at 6500** instead of 0.780. The
enrichment faded over the last 400 rpm, on every slot, in the band holding three
of the lineage's -3.00 CRK events. R24 writes the same 0.780 onto the 1100.01
row in the same four columns:

    row          5504     5984     6496     7008
    1100.01 R23  0.830    0.810    0.800    0.800
    1100.01 R24  0.780    0.780    0.780    0.780

3008 rpm is deliberately excluded — WOT there runs 1382 mg/stk on the aggressive
curve and 1200 on the low-torque map, so the 1100.01 row is never reached under
load, and it holds 1.000. The pump has 25-31 points of headroom at the top
(`HPFP Eff Vol` median 75.6 % and 69.3 %), so this is the one enrichment in the
lineage that is not pump-limited. **Slot 1 is not held on that row**: the
offsets it would need deliver 0.7305 and 0.7510 under the wrong sign, past the
0.755 rich bound, and the both-signs guard refuses them — checked in the build,
not asserted in a comment.

**Five tables differ from R23**: slot 2's `PUT setpoint` — map slot boost cap,
slot 2's `Lambda modifier` — map slot lambda offset, and the three basic lambda
setpoint grids, in which **only row 5 moves, in four columns**. Slots 1, 3, 4
and 5 keep their boost caps, read off the R23 bin and written back. Base timing,
the knock loop, the wastegate and the limiters do not move, and neither does
anything R23 wrote on the 1200.01 and 1389.00 rows.

**R23 has not been flashed**, so the car still holds R22. Below ~6100 rpm R24
leaves slot 3 completely alone, so a slot 3 pull on an R24 bin is still a clean
test of R23's 3008 rpm enrichment and its +0.750 CRK at 3500/4000 rpm. Above
~6100 rpm the two revisions are the *same* intervention — R24 is what makes
R23's 0.780 actually arrive — so that band tests the pair together rather than
either alone, which is the honest reading and not a confound worth avoiding.

**Slots 4 and 5 are for a dosed tank only** — pump 92 AKI dosed with VP Octanium
Unleaded at 10-11 oz per 10 US gallons. **Slot 3 remains the everyday map and
the in-drive fallback.** Slot 2 is pump 92 and safe on any tank.

This is a starting point for human review and logging, not a finished
calibration, and the script never flashes. See `Tunes/REV_LOG.md` § R24 for the
logging gate.
"""
...
def _apply_r24_lambda_routing(tune: Tune) -> dict[int, np.ndarray]:
    """Hold slot 1 at its prior lambda; release slot 2 to the enrichment.

    R23 introduced the patch's `Lambda modifier` — map slot lambda offset grids
    to this lineage and used them on slots 1 and 2. The routing was chosen for
    its failure mode and that reasoning is unchanged: the enrichment lives in
    the shared base grid, a path logged since R00, and the modifier is used only
    to take it *back off* a low-boost slot, so every way the never-yet-observed
    sign can be wrong is a rich failure on a low-demand map.

    R24 drops slot 2 from that set. Its new boost curve is clamped at slot 3's
    above ~6150 rpm, so it now runs the aggressive curve's boost at the top of
    the range; holding it at the old 0.800 lambda there would hand the
    low-torque map the exact top-end condition R23 enriched slot 3 to fix. Its
    grid is declared back to the patch's neutral 0.00 instead — declared, not
    omitted, because the write stages no bytes on a freshly patched bin but does
    authorise the extent against the R23 bin, where those cells are non-zero.
    ...
    untouched_before = {
        ...
        if slot not in HOLD_LAMBDA_SLOTS and slot not in RELEASE_LAMBDA_SLOTS
    }
    for slot in HOLD_LAMBDA_SLOTS:
        tune.switchpatch.slot_lambda_map(
            ...
            intent=(
                ...
                "shared base grid goes richer for every other slot: slot 1 is "
                "the bad-tank map, the lowest boost in the ladder, and keeping "
                "its fuelling where it has been logged leaves an unenriched "
                "reference. Positive offsets only, so a wrong sign on this "
                "never-yet-observed grid makes this slot richer rather than "
                "leaner"
            ),
        )

    # Slot 2 back to neutral. Same call, all-zero offsets: delivered lambda is
    # then the base grid's own enriched value, which is the point.
    for slot in RELEASE_LAMBDA_SLOTS:
        tune.switchpatch.slot_lambda_map(
            slot,
            rpm=rpm,
            rows={LAMBDA_LOAD[row]: tuple(0.0 for _ in rpm)
                  for row in LAMBDA_LOADED_ROWS},
            delivered_lambda_range=DELIVERED_LAMBDA_RANGE,
            base_grid="lambda_basic_hpdi",
            require_as_patched=True,
            intent=(
                f"release map slot {slot} to the shared base grid's enrichment "
                "by declaring its `Lambda modifier` — map slot lambda offset "
                "grid at the patch's neutral 0.00. R23 held this slot at the "
                "leaner lambda it ran under R22; R24's low-torque curve is "
                "clamped at slot 3's boost above ~6150 rpm, so a map running "
                "the aggressive curve's boost gets the aggressive curve's "
                "fuelling. Declared rather than omitted so the audit can "
                "attribute the difference from the R23 bin"
            ),
        )
    ...
    for slot, before in untouched_before.items():
        ...
        if not np.array_equal(before, after):
            raise RuntimeError(
                ...
                f"holding slot {HOLD_LAMBDA_SLOTS} back. Slot {EVERYDAY_SLOT} "
...
def declare(tune: Tune) -> tuple[float, tuple[float, float],
                                 ...
                                 dict[int, np.ndarray]]:
    """Declare the complete R24 calibration.
    ...
    offsets holding slot 1 back — the numbers a human reads before
    flashing.
    """
    peak_gap, max_gap = _check_slot_ladder()
    _check_row5_hold_refused()
    ...
    lambda_holds = _apply_r24_lambda_routing(tune)
...
def main() -> None:
    if not R23_REFERENCE.is_file():
        raise SystemExit(f"Missing the verified R23 reference bin: {R23_REFERENCE}")
    reference_hash = _sha256(R23_REFERENCE)
    if reference_hash != R23_REFERENCE_SHA256:
        raise SystemExit(
            f"R23 reference hash mismatch: {reference_hash}; "
            f"expected {R23_REFERENCE_SHA256}"
        )
    ...
    result = build(
        ...
        reference_bin=R23_REFERENCE,
        title="TUNE_MainTune_R24 — rebuild map slot 2 as the low-torque map: a "
              "flat 1200 mg/stk airmass target, which is a power curve linear "
              "in rpm, peaking at ~6250 rpm instead of 5500",
        summary=SUMMARY,
    )
    ...
    print(f"R24 saved  : {result.bin_path}")
    print(f"R24 report : {result.report_path}")
    print(f"R24 journal: {len(result.journal)} entries; "
          f"{len(result.journal.tables_touched())} tables touched")
    print(f"R24 audit  : {result.diff.summary()}")
    print(f"R24 Reference IGA migration max deviation: "
          f"{reference_iat_deviation:.6f}°CRK")
    print("R24 slot ladder (boost cap peak, absolute hPa / psi gauge) — "
          "only slot 2 moves:")
    for slot in sorted(SLOT_CURVES):
        curve = SLOT_CURVES[slot]
        peak = float(np.max(curve))
        print(f"  slot {slot}: {peak:7.0f} hPa  {(peak - 1013.25) / 68.9476:5.2f} psi"
              f"  — {SLOT_LABELS[slot]}")
    print(f"R24 octane slots separate by {peak_gap:.0f} hPa at peak, "
          f"{max_gap:.0f} hPa maximum ({max_gap / 68.9476:.2f} psi)")

    print("R24 map slot 2, the low-torque curve "
          f"(flat {LINEAR_TARGET_MG:.0f} mg/stk target):")
    print("  rpm      " + "".join(f"{v:>8.0f}" for v in SLOT_RPM_AXIS))
    print("  R23 hPa  " + "".join(f"{v:>8.0f}" for v in _R23_SLOT2_CONSERVATIVE))
    print("  R24 hPa  " + "".join(f"{v:>8.0f}" for v in SLOT_CURVES[2]))
    print("  d psi    " + "".join(
        f"{(n - o) / 68.9476:>8.2f}"
        for o, n in zip(_R23_SLOT2_CONSERVATIVE, SLOT_CURVES[2])))
    clamped = SLOT_CURVES[2] >= SLOT_CURVES[CONTROL_SLOT] - 1e-9
    print("  clamped  " + "".join(
        f"{('slot3' if c else ''):>8}" for c in clamped)
        + "   <- where holding the target would need unlogged boost")
    print(f"  slot 2 peaks at {SLOT_CURVES[2].max():.0f} hPa, below slot 1's "
          f"{SLOT_CURVES[1].max():.0f} hPa; at or below slot "
          f"{CONTROL_SLOT} at every breakpoint")

    enriched = tuple(sorted(LAMBDA_R23_ENRICHED))
    print("R24 lambda, loaded rows (1200.01 / 1389.00 mg/stk) — R23's, unchanged:")
    ...
    top_row = LAMBDA_R24_TOP_END_ROW
    print(f"R24 top-end enrichment extended to the "
          f"{LAMBDA_LOAD[top_row]:.2f} mg/stk row "
          f"(R23 wrote only 1200.01 / 1389.00, and WOT filling falls below "
          f"1200 above ~6100 rpm):")
    print("  rpm       " + "".join(f"{v:>9.0f}" for v in LAMBDA_R24_TOP_END_RPM))
    print(f"  {LAMBDA_LOAD[top_row]:>7.2f} R23"
          + "".join(f"{LAMBDA_CELLS_R22[top_row][LAMBDA_RPM.index(int(v))]:>9.3f}"
                    for v in LAMBDA_R24_TOP_END_RPM))
    print(f"  {LAMBDA_LOAD[top_row]:>7.2f} R24"
          + "".join(f"{LAMBDA_CELLS[top_row][LAMBDA_RPM.index(int(v))]:>9.3f}"
                    for v in LAMBDA_R24_TOP_END_RPM))
    print("  slot 1 is NOT held on this row: the offsets it would need deliver "
          + ", ".join(f"{v:.4f}" for v in _check_row5_hold_refused())
          + f" under the wrong sign, past the {DELIVERED_LAMBDA_RANGE[0]} rich "
            "bound, so the both-signs guard refuses them")

    for slot in RELEASE_LAMBDA_SLOTS:
        print(f"  slot {slot} `Lambda modifier` released to the patch's neutral "
              f"0.00 — it takes the base grid's enrichment with slots "
              f"{CONTROL_SLOT}, {MID_BOOSTER_SLOT} and {BOOSTER_SLOT}")
    print(f"  delivered lambda bounded to {DELIVERED_LAMBDA_RANGE} under both "
          f"sign conventions")

    for slot in OCTANE_SLOTS:
        print(f"R24 slot {slot} delivered timing at 1400 mg/stk (°CRK): "
              + ", ".join(f"{rpm:g}:{deg:+.3f}"
                          for rpm, deg in zip(SPARK_MODIFIER_RPM,
                                              delivered_by_slot[slot])))
    print(f"R24 slot {EVERYDAY_SLOT} delivered timing at 1400 mg/stk (°CRK): "
          + ", ".join(f"{rpm:g}:{deg:+.3f}"
                      for rpm, deg in zip(SPARK_MODIFIER_RPM,
                                          everyday_delivered)))
    print(f"R24 slot {EVERYDAY_SLOT} ignition offset: "
          + ", ".join(f"{rpm:g}:{deg:+.3f}"
                      for rpm, deg in zip(R23_SPARK_RPM, R23_SPARK_DEG))
          + " °CRK, and neutral in every other cell")

    print(
        f"\nReview the R23-to-R24 comparison plots and "
        f"MainTune_out/R24_changes_summary.png before any human-performed CAL "
        f"flash. Exactly FIVE tables may differ from R23: map slot 2's "
        f"`PUT setpoint` — map slot boost cap; map slot 2's `Lambda modifier` — "
        f"map slot lambda offset (which returns to the patch's neutral 0.00); "
        f"and the three basic lambda setpoint grids (`IP_LAMB_BAS[1]`, "
        f"`IP_LAMB_BAS_HPDI[1]` and `IP_LAMB_BAS_MPI[1]`), which take R23's "
        f"0.780 top-end value down onto the "
        f"{LAMBDA_LOAD[LAMBDA_R24_TOP_END_ROW]:.2f} mg/stk row as well.\n"
        f"\nIn those three grids, ONE ROW MOVES — row "
        f"{LAMBDA_R24_TOP_END_ROW} — in "
        f"{len(LAMBDA_R24_TOP_END_RPM)} columns. Everything R23 wrote on the "
        f"1200.01 and 1389.00 rows must be UNCHANGED. R23's enrichment faded "
        f"out above ~6100 rpm because WOT filling falls off the 1200.01 "
        f"breakpoint (1176 mg/stk at 6250 rpm, 1089 at 6500), so the command "
        f"came out 0.786 and 0.801 instead of 0.780. This closes that, on every "
        f"slot.\n"
        f"\nThe OTHER FOUR `PUT setpoint` — map slot boost caps must show NO "
        f"CHANGE AT ALL, and neither must the nine "
        f"`IP_IGA_BAS_IVVT_VVL_PORT_L[STND][i][e]` — Basic ignition angle maps, "
        f"the knock fast-loop tables, the wastegate feedforward, slot 1's "
        f"`Lambda modifier`, or any `Spark modifier` — map slot ignition offset "
        f"grid. Anything else in the diff is a bug.\n"
        f"\nTHE LADDER IS NO LONGER MONOTONIC IN BOOST. Slot 2 now sits well "
        f"below slot 1 through the mid-range and above it from ~6000 rpm, so "
        f"the 'slot 1 is lowest everywhere' invariant every revision since R14 "
        f"carried is retired. Three replacements are asserted before any write: "
        f"slot 1 stays at or below slots {CONTROL_SLOT}, {MID_BOOSTER_SLOT} and "
        f"{BOOSTER_SLOT} everywhere; slot 2 stays at or below slot "
        f"{CONTROL_SLOT} everywhere; and slot 2's peak stays below slot 1's. "
        f"This is the point to review most carefully.\n"
        f"\nR23 has not been flashed, so the car holds R22. Below ~6100 rpm "
        f"R24 leaves slot {CONTROL_SLOT} completely alone, so a slot "
        f"{CONTROL_SLOT} pull on this bin is still a clean test of R23's 3008 "
        f"rpm enrichment and its +0.750 CRK at 3500/4000 rpm. Above ~6100 rpm "
        f"the two are the SAME intervention — R24 is what makes R23's 0.780 "
        f"actually arrive — so that band tests the pair together.\n"
        ...
        f"everyday map and the in-drive fallback; slot 2 is pump 92 and safe on "
        f"any tank.\n"
        f"\nSee Tunes/REV_LOG.md § R24 for the logging gate. Slot 2 wants a "
        f"3rd-gear pull from below 3000 rpm to redline, logging `Airmass`, "
        f"`PUT`, `PUT SP` and rpm: the claim to falsify is that airmass holds "
        f"flat at {LINEAR_TARGET_MG:.0f} mg/stk from ~3400 rpm to ~6100 rpm. If "
        f"it sags in the middle the VE measurement is wrong; if it overshoots, "
        f"the cap is not binding where it was assumed to.\n"
