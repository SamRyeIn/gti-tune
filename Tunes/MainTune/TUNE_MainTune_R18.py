#!/usr/bin/env python3
"""MainTune R18 — local timing correction from R17 knock evidence.

R18 inherits the complete R17 calibration and patch set. Relative to R17, it
changes four cells in each of the nine
`IP_IGA_BAS_IVVT_VVL_PORT_L[STND][i][e]` — Basic ignition angle, VVL 0
port-flap-low cam-position maps: 4500 rpm is set to −3.75° and 5000 rpm to
−2.25° at both 1200 and 1400 mg/stk. The correction is bounded by the prior R04
protection and responds to three R17 pulls with −3.0° knock retard in the
4563–4973 rpm / 1480–1511 mg/stk region. Every other calibration area remains
identical to R17. This script builds a human-review candidate and never flashes
an ECU.
...
OUT_BIN_NAME = "Patched_259L_R18.bin"

R17_REFERENCE = (
    REPO_ROOT / "Tunes" / "MainTune" / "MainTune_out"
    / "R17_20260826-120335"
    / "Patched_259L_R17.bin"
)
R17_REFERENCE_SHA256 = (
...
SUMMARY = """\
R18 retains the tuning-guide author's exact Spark IAT calibration in
...
R18 responds to three R17 pulls with −3.0° knock retard at 4563–4973 rpm and
1480–1511 mg/stk. It sets 4500 rpm to −3.75° and 5000 rpm to −2.25° at both
1200 and 1400 mg/stk in all nine
`IP_IGA_BAS_IVVT_VVL_PORT_L[STND][i][e]` — Basic ignition angle, VVL 0
port-flap-low cam-position maps. Relative to R17, that is a 0.75° pull at 4500
rpm and a 1.50° pull at 5000 rpm. The values use the ECU's exact 0.375°CRK
encoding step and stay within the prior R04 protection bounds. Stock knock
detection and correction behavior remain untouched.

Every Spark-IAT, boost, wastegate, fueling, limiter, slot, and patch declaration
remains identical to R17. This is a starting point for human review and logging,
...
def declare(tune: Tune) -> float:
    """Declare the complete R18 calibration and return Reference-IAT deviation."""
...
def main() -> None:
    if not R17_REFERENCE.is_file():
        raise SystemExit(f"Missing the verified R17 reference bin: {R17_REFERENCE}")
    reference_hash = _sha256(R17_REFERENCE)
    if reference_hash != R17_REFERENCE_SHA256:
        raise SystemExit(
            f"R17 reference hash mismatch: {reference_hash}; "
            f"expected {R17_REFERENCE_SHA256}"
        )
    ...
    result = build(
        ...
        reference_bin=R17_REFERENCE,
        title="TUNE_MainTune_R18 — local timing correction from R17 logs",
        summary=SUMMARY,
    )
    ...
    print(f"R18 saved  : {result.bin_path}")
    print(f"R18 report : {result.report_path}")
    print(f"R18 journal: {len(result.journal)} entries; "
          f"{len(result.journal.tables_touched())} tables touched")
    print(f"R18 audit  : {result.diff.summary()}")
    print(f"R18 Reference IGA migration max deviation: "
          f"{reference_iat_deviation:.6f}°CRK")
    print(
        "\nReview all nine "
        "`IP_IGA_BAS_IVVT_VVL_PORT_L[STND][i][e]` — Basic ignition angle, "
        "VVL 0 port-flap-low cam-position R17-to-R18 comparison plots before "
